...
//Almost Carbon Copy of net.minecraft.client.gui.components.EditBox.
//Because we need to change the rendering, which depends on a lot of private fields and methods. So we cannot simply inherit the class.
...
public class CallInSeqBox extends AbstractWidget implements Renderable {
   ...
   public CallInSeqBox(Font pFont, int pX, int pY, int pWidth, int pHeight, Component pMessage) {
      this(pFont, pX, pY, pWidth, pHeight, (EditBox)null, pMessage);
   }

   public CallInSeqBox(Font pFont, int pX, int pY, int pWidth, int pHeight, @Nullable EditBox pEditBox, Component pMessage) {
     ...
     * there is currently a selection, in
     ...
     * unless there is currently a selection,
    ...
    public void renderWidget(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        if (this.isVisible()) {
            if (this.isBordered()) {
                ...
                pGuiGraphics.fill(this.getX() - 1, this.getY() - 1, this.getX() + this.width + 1,
                        this.getY() + this.height + 1, i);
                pGuiGraphics.fill(this.getX(), this.getY(), this.getX() + this.width, this.getY() + this.height,
                        -16777216);
            }
            ...
            String sequenceValue = OrbitalLaserAttackPattern.replaceWithArrows(this.value);
            String s = this.font.plainSubstrByWidth(sequenceValue.substring(this.displayPos), this.getInnerWidth());
            ...
            boolean flag2 = this.cursorPos < sequenceValue.length() || sequenceValue.length() >= this.getMaxLength();
            int k1 = j1;
            if (!flag) {
     ...
     * longer than this length, the current
    ...
    public boolean isMouseOver(double pMouseX, double pMouseY) {
        return this.visible && pMouseX >= (double) this.getX() && pMouseX < (double) (this.getX() + this.width)
                && pMouseY >= (double) this.getY() && pMouseY < (double) (this.getY() + this.height);
    }
     ...
     * cursor position mark the edges of the
     * selection). If the anchor is set beyond the bounds of the current text, it
    ...
    public int getScreenX(int pCharNum) {
        return pCharNum > this.value.length() ? this.getX()
                : this.getX() + this.font.width(this.value.substring(0, pCharNum));
    }
