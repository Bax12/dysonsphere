...
public class HeatExchangerCategory implements IRecipeCategory<HeatExchangerRecipe> {
    ...
    public List<Component> getTooltipStrings(HeatExchangerRecipe recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
        if(mouseX >= 29 && mouseX <= 38 && mouseY >= 2 && mouseY <= 34){
            ...
            tooltips.add(Component.translatable("tooltip.dysonsphere.heat_exchanger_min_heat", AssetUtil.FLOAT_FORMAT.format(recipe.minHeat())));
            if(recipe.heatScaling() != 0 && recipe.scalingFactor() != 0){
                tooltips.add(Component.translatable("tooltip.dysonsphere.heat_exchanger_heat_bonus", AssetUtil.FLOAT_FORMAT.format(recipe.scalingFactor() * 100), AssetUtil.FLOAT_FORMAT.format(recipe.heatScaling())));
            }
            return tooltips;
        }
        return List.of();
