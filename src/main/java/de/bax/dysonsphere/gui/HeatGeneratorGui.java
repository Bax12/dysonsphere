...
public class HeatGeneratorGui extends BaseGui<HeatGeneratorContainer> {
    ...
    protected void init() {
        ...
        this.heat = new HeatDisplay(this.leftPos + 7, this.topPos + 5, tile.internalHeat);
        ...
        if(tile.getAxis() != -1){
            ...
            // }
            n1Handler = tile.sideHeat[tile.getAxis()*2];
            n2Handler = tile.sideHeat[(tile.getAxis()*2)+1];
            axis = Direction.values()[tile.getAxis()*2].getAxis().getName().toUpperCase();//direction is sorted yxz, axis xyz
        }
        ...
        this.n2Heat = new HeatDisplay(this.leftPos + 118, this.topPos + 5, n2Handler){
            ...
            protected void addTooltip(List<Component> tooltip) {
                super.addTooltip(tooltip);
                tooltip.add(Component.translatable("tooltip.dysonsphere.heat_generator_neighbor_pos", axis));
            }
        };
    }
    ...
    protected void renderBg(@Nonnull GuiGraphics guiGraphics, float f, int x, int y) {
        ...
        if(this.tile.getHeatDifference() > HeatGeneratorTile.minHeatDifference){
            guiGraphics.blit(RES_LOC, this.leftPos + 82, this.topPos + 36, 176, 0, 12, 21);
        }
        
        guiGraphics.drawCenteredString(font, Component.translatable("tooltip.dysonsphere.heat_generator_axis", (!axis.isEmpty() ? axis : "NONE")), this.leftPos + 88, this.topPos + 23, 0xFFFFFFFF);
