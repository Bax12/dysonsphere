...
public class DSMonitorRenderer implements BlockEntityRenderer<DSMonitorTile> {
    ...
    public void render(@Nonnull DSMonitorTile tile, float partialTicks, @Nonnull PoseStack poseStack, @Nonnull MultiBufferSource bufferSource, int combinedLight, int combinedOverlay) {
        ...
        if(tile.getDsCompletionPercentage() == -1){
            poseStack.pushPose();
            var comp = Component.translatable("tooltip.dysonsphere.ds_monitor_offline1");
            float scale = Math.min(125F / font.width(comp), 1f);
            poseStack.scale(scale, 1, 1);
            matrix = poseStack.last().pose();
            font.drawInBatch(comp, 0, 10F, -1, false, matrix, bufferSource,DisplayMode.NORMAL, j, combinedLight);
            poseStack.popPose();
            poseStack.pushPose();
            comp = Component.translatable("tooltip.dysonsphere.ds_monitor_offline2");
